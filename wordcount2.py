...
def make_dict_of_word_count(list_of_words):
    ...
    for word in list_of_words:
        if len(list_of_non_duplicate_words) > 0 and list_of_non_duplicate_words[-1] == word:
            ...
            if len(list_of_non_duplicate_words) > 0:
...
def print_top_20(word_count_dict):
    """
    Function that takes the dictionary of word counts and sorts them by their values from highest to lowest. In
    essence, getting a "top 20" list of the most-used words in the file
    :param word_count_dict dict: dictionary of the word count of a file
    """
    sorted_list = sorted(word_count_dict.items(), key=operator.itemgetter(1), reverse=True)
    i = 0
    while i < 20:
        print '{} {}'.format(str(sorted_list[i][0]), str(sorted_list[i][1]))
        i += 1
